---
title: " 테이블 그리드 구성 요소 | Microsoft Docs"
description: 테이블 그리드 구성 요소 구현
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340210"
---
# <a name="implementing-table-grid-component"></a>테이블 그리드 구성 요소 구현

이 샘플에서는 폼에 대 한 페이지 및 레코드 탐색에서 간단한 데이터 집합 구성 요소, 보기의 열 메타 데이터 바인딩, 레코드 바인딩, 추가 레코드를 만드는 방법을 보여 줍니다.
구성 요소 헤더 열과 내부 레코드 값은 기존 뷰에 바인딩됩니다.

> [!div class="mx-imgBorder"]
> ![테이블 그리드 구성 요소](../media/table-grid-control.png "테이블 그리드 구성 요소")
...
열 정보 보기는 `context.parameters.[dataset_property_name].columns` 있습니다. 배열 형식입니다.

레코드 바인딩:

- 정렬 된 레코드 Id는 `context.parameters.[dataset_property_name].sortedRecordIds`
- 모든 레코드 정보는 `context.parameters.[dataset_property_name].records`에 있습니다. 
- 각 레코드 개체에 대해 `context.parameters.[dataset_property_name].records[record_Id]` 
- 형식이 지정 된 값은 `getFormattedValue`에서 검색할 수 있습니다. 

필요한 경우 더 많은 데이터 페이지 로드:

`context.parameters.[dataset_property_name].paging` `hasNextPage` 및 `loadNextPage` 데이터와 같은 페이징 기능을 제공 합니다. 다음 페이지 데이터가 있으면 `Load More` 단추가 표시 됩니다.

또한이 샘플에서는 구성 요소가 컨테이너 크기 조정에 대해 수신 하는 방법을 보여 줍니다. 

[Updateview](../reference/control/updateview.md) 가 호출 될 때마다 `mode.allocatedWidth` 및 `mode.allocatedHeight` 제공 되도록 [init](../reference/control/init.md) 메서드 내에서 `trackContainerResize` 메서드를 호출 해야 합니다. 이 메서드가 처음에 호출 되지 않는 경우에는 `allocatedWidth` 및 `allocatedHeight` 제공 되지 않습니다.

AllocatedHeight가 – 1 이면 높이에 제한이 없음을 의미 합니다. 구성 요소는 제공 된 너비에 따라 높이를 조정 해야 합니다.
