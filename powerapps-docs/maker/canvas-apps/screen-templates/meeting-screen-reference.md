...
* 속성 **색상**<br>
    ...
    **_Showdetails** 는 **LblInviteTab** 컨트롤 또는 **LblScheduleTab** 컨트롤이 선택 되었는지 여부를 확인 하는 데 사용 되는 변수입니다. **_Showdetails** 값이 **true**이면 **LblScheduleTab** 가 선택 됩니다. 값이 **false**이면 **LblInviteTab** 가 선택 됩니다. 즉, **_Showdetails** 값이 **true** (이 탭이 선택 *되지* 않은 경우) 인 경우 탭 색은 **LblRecipientCount**의 값과 일치 합니다. 그렇지 않으면 **RectQuickActionBar**의 fill 값과 일치 합니다.
...
* 속성 **색상**<br>
    ...
    **_Showdetails** 는 **LblInviteTab** 컨트롤 또는 **LblScheduleTab** 컨트롤이 선택 되었는지 여부를 확인 하는 데 사용 되는 변수입니다. True 이면 **LblScheduleTab** 가 선택 되 고, false 이면 **LblInviteTab** 가입니다. 즉, **_Showdetails** 가 true 이면 (이 탭이 선택 *됨* ) 탭 색은 **RectQuickActionBar**의 fill 값과 일치 합니다. 그렇지 않으면 **LblRecipientCount**의 색 값과 일치 합니다.
...
* 속성 **되**<br>
    기본값 컨트롤이 표시 되려면 세 가지 논리적 검사가 모두 **true** 로 평가 되어야 합니다.
...
* 속성 **OnSelect**<br> 
    기본값 사용자를 참석자 목록에 추가 하 고, 사용 가능한 모임 시간을 새로 고치 며, 여러 변수를 설정/해제 하는 **Collect** 문입니다.
  ...
  이 컨트롤을 선택 하면 유효한 전자 메일 주소 (올바른 전자 메일 주소를 **Textsearchbox**에 입력 한 경우에만 표시 됨)가 **MyPeople** 컬렉션 (이 컬렉션은 참석자 목록)에 추가 되 고, 새 모임 시간이 사용자 항목입니다.
  ...
  1. **_LoadMeetingTimes** context 변수를 **true**로 설정 합니다. 이 변수는 로딩 상태를 설정 합니다 .이는 **_LblTimesEmptyState** 같은 로드 상태 컨트롤의 표시 여부를 설정 하 여 사용자에 게 데이터를 로드 하 고 있음을 표시 합니다.
  1. **_SelectedMeetingTime** 를 **Blank ()** 로 설정 합니다. **_selectedMeetingTime** 는 **FindMeetingTimesGallery** 컨트롤에서 선택 된 레코드입니다. 다른 참석자를 추가 하면 해당 참석자에 대해 이전 **_selectedMeetingTime** 정의를 사용할 수 없다는 것을 의미할 수 있습니다.
  1. **Selectedroom** 을 **Blank ()** 로 설정 합니다 (_f). **_Selectedroom** 은 **RoomBrowseGallery**에서 선택한 대화방 레코드입니다. 방 전반는 **_selectedMeetingTime**의 값에서 결정 됩니다. 이 값이 숨겨진 경우 **_Selectedroom** 값은 더 이상 유효 하지 않으므로 숨겨진 상태 여야 합니다.
  1. **_RoomListSelected** 을 **false**로 설정 합니다. 이 줄은 모든 사용자에 게 적용 되지 않을 수 있습니다. Office에서는 다른 "방 목록"을 기준으로 대화방을 그룹화 할 수 있습니다. 방 목록이 있는 경우이 화면에는 해당 목록에서 공간을 선택 하기 전에 먼저 방 목록을 선택할 수 있는이 화면이 표시 됩니다. **_RoomListSelected** 의 값은 사용자 (대화방 목록만 있는 테 넌 트)가 대화방 목록 또는 방 목록 내에서 대화방을 볼 지 여부를 결정 하는 내용입니다. 사용자가 새 대화방 목록을 다시 선택 하도록 하려면 **false** 로 설정 됩니다.
  ...
  1. **Clearcollect** 함수는 두 개의 열도 추가 합니다. "StartTime" 및 "EndTime"이 있습니다. 이렇게 하면 반환 되는 데이터가 간단해 집니다. 
  ...
  1. 이러한 함수가 모두 완료 되 면 **_loadingMeetingTimes** 변수가 **false**로 설정 되 고 로드 상태가 제거 되며 **_showMeetingTimes** 가 **true**로 설정 되어 **FindMeetingTimesGallery**가 표시 됩니다.
...
공백에 대 한 사용자 검색이 유효 하지 않으므로 **Textsearchbox** 가 **Trim** 함수에 래핑됩니다. 사용자가 검색 하기 전에 검색 결과를 검색 하는 것은 성능 낭비 이기 때문에 `Office365Users.SearchUser` 연산은 `If(!IsBlank(Trim(TextSearchBox.Text)) ... )` 함수에 래핑됩니다.
...
* 속성 **OnSelect**<br>
    ...
    1. **_LoadMeetingTimes** 상태를 **true** 로 설정 하 고 **_showMeetingTimes** 상태를 **False**로 설정 하 고 **_selectedMeetingTime** 및 _Ststststm 변수를 공백으로 지정 하 고 **MeetingTimes** 를 새로 고칩니다. **MyPeople** 컬렉션에 새 추가를 포함 하는 컬렉션입니다. 
    1. **_LoadMeetingTimes** 상태를 **false**로 설정 하 고 **_showMeetingTimes** 를 **true**로 설정 합니다. 선택 항목이 이미 **MyPeople** 컬렉션에 있으면 **textsearchbox**의 내용만 다시 설정 합니다.
...
* 속성 **높이로**<br>
    기본값 갤러리의 최대 높이를 350으로 늘릴 수 있도록 하는 논리:
   ...
   이 갤러리의 높이는 갤러리의 항목 수에 맞게 조정 되며 최대 높이는 350입니다. 수식은 **MeetingPeopleGallery**의 단일 행 높이로 76를 사용 하 고 행 수를 곱합니다. **WrapCount** 속성은 2로 설정 되므로 true @no__t 행의 수는-1입니다.
...
* 속성 **OnSelect**<br>
    기본값 참석자 목록에서 사용자를 제거 하는 **remove** 문, 사용 가능한 모임 시간을 새로 고치는 **Collect** 문 및 여러 변수를 전환 합니다.
  ...
  1. **_LoadMeetingTimes** 상태를 **true** 로 설정 하 고 **_showMeetingTimes** 상태를 **False**로 설정 하 고 **_selectedMeetingTime** 및 _Ststststm 변수를 공백으로 지정 하 고 **MeetingTimes** 를 새로 고칩니다. **MyPeople** 컬렉션에 새 추가를 포함 하는 컬렉션입니다. 
  1. **_LoadMeetingTimes** 상태를 **false**로 설정 하 고 **_showMeetingTimes** 를 **true**로 설정 합니다.
...
* 속성 **OnSelect**<br>
    기본값 사용 가능한 회의 시간을 새로 고치는 **Collect** 문과 여러 변수를 전환 합니다.
  ...
  1. **_LoadMeetingTimes** 상태를 **true** 로 설정 하 고 **_showMeetingTimes** 상태를 **False**로 설정 하 고 **_selectedMeetingTime** 및 _Ststststm 변수를 공백으로 지정 하 고 **MeetingTimes** 를 새로 고칩니다. 새 날짜 선택이 포함 된 컬렉션입니다. 
  1. **_LoadMeetingTimes** 상태를 **false**로 설정 하 고 **_showMeetingTimes** 를 **true**로 설정 합니다.
...
* 속성 **텍스트**<br>
    기본값 사용자의 현지 시간으로 표시 되는 시작 시간의 변환입니다.
...
* 속성 **OnSelect**<br>
    기본값 여러 변수를 설정/해제 하는 것은 물론, 회의실 및 제안 된 전반을 수집 하는 여러 가지 **수집** 문이 있습니다.
  ...
  낮은 수준에서이 코드 블록은 다음과 같습니다.
  1. **_SelectedMeetingTime** 을 선택한 항목으로 설정 합니다. 이는 해당 시간 동안 사용할 수 있는 대화방을 찾는 데 사용 됩니다.
  1. 로드 상태 변수 **_loadingRooms** 를 **true**로 설정 하 여 로드 상태를 설정 합니다.
  ...
  1. 사용자에 게 대화방 목록 또는 하나의 방 목록이 없는 경우:
      ...
      1. @No__t-0 작업은 테 넌 트의 첫 100 대화방을 검색 하는 데 사용 됩니다. 이러한 컬렉션은 **Allrooms** 컬렉션에 저장 됩니다.
      ...
      1. **RoomTimeSuggestions** 컬렉션은 [Office365](https://docs.microsoft.com/connectors/office365/#find-meeting-times) 를 사용 하 여 **_selectedMeetingTime** 변수의 시간 값을 기반으로 **allrooms** 컬렉션에서 처음 20 개 대화방의 전반를 검색 합니다. @No__t-0은 **DateTime** 값의 형식을 올바르게 지정 하는 데 사용 됩니다.
      1. **AvailableRooms** 컬렉션이 생성 됩니다. 이는 두 개의 추가 열이 추가 된 참석자 전반 **RoomTimeSuggestions** 컬렉션입니다. "Address" 및 "Name"입니다. "Address"는 대화방의 메일 주소이 고 "Name"은 대화방의 이름입니다.
      ...
      1. **_roomListSelected** 가 **false**로 설정 되어 있습니다.
  1. 로드 상태 ( **_loadingRooms**)는 다른 모든 항목의 실행이 완료 되 면 **false** 로 설정 됩니다.
...
* 속성 **품목이**<br>
    기본값 사용자가 대화방 목록을 선택 했거나 테 넌 트에 대화방 목록이 있는지 여부에 따라 논리적으로 동일한 스키마의 두 개의 내부 컬렉션으로 설정 됩니다.
  ...
  이 갤러리는 **_roomListSelected** 또는 **_Noroomlists** 가 **true**인 경우 **AvailableRoomsOptimal** 컬렉션을 표시 합니다. 그렇지 않으면 **RoomsLists** 컬렉션을 표시 합니다. 이러한 컬렉션의 스키마가 동일 하기 때문에이 작업을 수행할 수 있습니다.
...
* 속성 **OnSelect**<br>
    기본값 사용자가 대화방 목록을 볼 수 있는지 여부에 따라 논리적으로 바인딩된 **Collect** 및 **set** 문의 집합 (트리거될 수 있음):
  ...
  이 컨트롤이 선택 될 때 발생 하는 작업은 사용자가 현재 회의실 목록이 나 회의실 집합을 보고 있는지 여부에 따라 달라 집니다. 이 컨트롤을 선택 하면 선택 된 대화방 목록에서 선택한 시간에 사용할 수 있는 대화방을 검색 합니다. 후자 인 경우이 컨트롤을 선택 하면 **_Selectedroom** 변수가 선택한 항목으로 설정 됩니다. 위의 문은 [**FindMeetingTimesGallery**](#find-meeting-times-gallery)의 **Select** 문과 매우 유사 합니다.
  ...
  1. 대화방 목록이 선택 되었는지와 테 넌 트에 방 목록이 있는지 확인 합니다. 그렇다면 다음과 같이 합니다.
      1. **_RoomListSelected** 를 **true** 로 설정 하 고 **_selectedRoomList** 를 선택한 항목으로 설정 합니다.
      ...
      1. **RoomTimeSuggestions** 컬렉션은 FindMeetingTimes 작업을 사용 하 여 **_selectedMeetingTime** 의 시간 값을 기반으로 **allrooms** 컬렉션에서 처음 20 개 대화방의 전반를 검색 합니다 [.](https://docs.microsoft.com/connectors/office365/#find-meeting-times) 변수. **DateTime** 값의 형식을 올바르게 지정 하는 데 `& "Z"`이 사용 됩니다.
      1. **AvailableRooms** 컬렉션이 생성 됩니다. 이는 두 개의 추가 열이 추가 된 참석자 전반 **RoomTimeSuggestions** 컬렉션입니다. "Address" 및 "Name"입니다. "Address"는 대화방의 메일 주소이 고 "Name"은 대화방의 이름입니다.
      1. 그런 다음 **AvailableRoomsOptimal** 컬렉션을 만듭니다. "가용성" 및 "참석자" 열을 제거 하는 **AvailableRooms** 컬렉션입니다. 이렇게 하면 **AvailableRoomsOptimal** 및 **allrooms**의 스키마가 일치 합니다. 이렇게 하면 **RoomBrowseGallery**의 **Items** 속성에서 두 컬렉션을 모두 사용할 수 있습니다.
      1. **_roomListSelected** 가 **false**로 설정 되어 있습니다.
  1. 로드 상태 ( **_loadingRooms**)는 다른 모든 항목의 실행이 완료 되 면 **false** 로 설정 됩니다.
...
* 속성 **OnSelect**<br>
    ...
    **_RoomListSelected** 을 **false**로 설정 하면 **RoomBrowseGallery** 컨트롤이 **RoomsLists** collection의 항목을 표시 하도록 변경 됩니다.
...
* 속성 **DisplayMode**<br>
    기본값 아이콘을 편집 하기 전에 사용자가 특정 모임 세부 정보를 입력 하도록 강제 하는 논리입니다.
...
* 속성 **OnSelect**<br>
    ...
    기본값 선택한 참석자에 게 모임 초대를 보내고 모든 입력 필드를 지우시겠습니까 위한 코드입니다.
  ...
  낮은 수준에서이 코드 블록은 다음과 같습니다.
  1. [Office365 ()](https://docs.microsoft.com/connectors/office365/#get-calendars) 작업에서 **DisplayName** 이 "calendar" 인 일정에 **_mycalendarname** 을 설정 합니다.
