---
title: " Web API 구성 요소 | Microsoft Docs"
description: Web API 구성 요소 구현
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340164"
---
# <a name="implementing-web-api-component"></a>Web API 구성 요소 구현

Web API 구성 요소는 만들기, 검색, 업데이트 및 삭제 작업을 수행 하도록 설계 되었습니다. 구성 요소는 네 개의 단추를 렌더링 합니다 .이 단추를 클릭 하 여 다른 웹 API 작업을 호출할 수 있습니다. Web API 호출의 결과는 코드 구성 요소의 아래쪽에 있는 HTML div 요소에 삽입 됩니다.  

> [!div class="mx-imgBorder"]
> ![WEB api 구성 요소](../media/web-api-control.png "웹 api 구성 요소")
...
기본적으로이 샘플에서 구성 요소는 `Account` 엔터티에 대 한 만들기, 검색, 업데이트 작업을 수행 하 고 Web API 예제에서 이름 및 수익 필드를 설정 하도록 구성 됩니다.

엔터티 또는 필드에 대 한 기본 구성을 변경 하려면 아래와 같이 구성 값을 업데이트 합니다.  
...
@No__t_0 메서드는 세 개의 단추를 렌더링 하 여 수익 필드가 다른 값 (100, 200, 300)으로 설정 된 계정 레코드를 만들 수 있도록 합니다.

만들기 단추 중 하나를 클릭 하면 단추의 `onClick` 이벤트 처리기가 클릭 한 단추 값을 확인 하 고 Web API 작업을 사용 하 여 수익 필드가 단추의 값으로 설정 된 계정 레코드를 만듭니다. 계정 레코드의 이름 필드는 문자열의 끝에 임의의 `int` 추가 된 `Web API code component (Sample)`로 설정 됩니다. Web API 호출의 콜백 메서드는 웹 API 호출 (성공 또는 실패)의 결과를 사용자 지정 컨트롤의 결과 div에 삽입 합니다.  
 
@No__t_0 메서드는 클릭 하면 조회 대화 상자를 여는 단추를 렌더링 합니다. 조회 대화 상자에서 삭제 하려는 계정 레코드를 선택할 수 있습니다. 조회 대화 상자에서 계정 레코드를 선택 하면 데이터베이스에서 레코드를 삭제 하는 `deleteRecord` 전달 됩니다. Web API 호출의 콜백 메서드는 웹 API 호출 (성공 또는 실패)의 결과를 사용자 지정 컨트롤의 결과 div에 삽입 합니다.  

FetchXML `retrieveMultiple` 메서드는 코드 구성 요소에서 단추를 렌더링 합니다. 이 단추 `onClick` FetchXML이 생성 되 고 `retrieveMultiple` 함수로 전달 되어 모든 계정 레코드에 대 한 수익 필드의 평균 값을 계산 합니다. Web API 호출의 콜백 메서드는 웹 API 호출 (성공 또는 실패)의 결과를 사용자 지정 컨트롤의 결과 div에 삽입 합니다.  

OData `retrieveMultiple` 메서드는 코드 구성 요소에서 단추를 렌더링 합니다. 이 단추를 `onClick` OData 문자열이 생성 되 고 `retrieveMultiple` 함수로 전달 되어 모든 계정 레코드를 검색 하는 데 사용 됩니다. 예를 들어,이 코드 구성 요소에서 만든 모든 계정 레코드에 대해 true 인 ' 코드 구성 요소 Web API (샘플) '와 같은 이름 필드를 사용 하 여 모든 계정 레코드를 검색 합니다.  

레코드를 성공적으로 검색 하는 경우 코드 구성 요소는 수익 필드가 100, 200 또는 300로 설정 된 계정 레코드 수를 계산 하는 논리를 포함 하 고 코드 구성 요소에서이 수를 odata 상태 컨테이너 div에 표시 합니다.  Web API 호출의 콜백 메서드는 웹 API 호출 (성공 또는 실패)의 결과를 사용자 지정 컨트롤의 결과 div에 삽입 합니다.  
