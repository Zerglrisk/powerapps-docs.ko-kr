...
기본 제공 색 값을 사용 하 여, 사용자 지정 색 및 알파 채널을 사용 합니다.

## <a name="description"></a>설명

사용 하 여는 **Color** 열거형에 쉽게 액세스할 수 있습니다 하 여 HTML의 스타일 시트 (CSS)에 정의 된 색. 예를 들어 **Color.Red** 순수한 빨간색을 반환 합니다. 이 항목의 끝에 있는 이러한 색 목록을 찾을 수 있습니다.

합니다 **ColorValue** CSS 색 문자열에 따라 색을 반환 합니다. 문자열을 이러한 형식 중 하나를 사용할 수 있습니다.

- **CSS 색 이름:** **"RoxyBrown"** 하 고 **"OliveDrab"** 은 예입니다. 이러한 이름은 공백을 포함 하지 않습니다. 이 항목의 뒷부분에 나오는 지원 되는 색 목록이 표시 됩니다.
- **6 자리 16 진수 값:** 예를 들어 **"#ffd700"** 와 같습니다 **"Gold"** 합니다. 형식에서 문자열은 "#*rrggbb*" 위치 *rr* 은 두 자리 16 진수의 빨간색 부분 *gg* 는 녹색, 및 *bb* 파란색은입니다.
- **8 자리 16 진수 값:** 예를 들어 **"#ff7f5080"** 와 같습니다 **"산호색"** 50% 알파 채널을 포함 합니다. 형식에서 문자열은 "#*rrggbbaa*" 위치 *rr*를 *gg*, 및 *bb* 6 자리 형식을 동일 합니다. 알파 채널은 표현 *aa*: **00** 나타냅니다 완전히 투명 하 고, 및 **ff** 나타냅니다 완전히 불투명 합니다.

합니다 **RGBA** 빨간색, 녹색 및 파랑 구성 요소에 따라 색을 반환 합니다. 함수에는 서로 앞에 계층에 있는 컨트롤의 색을 혼합 알파 채널이 포함 됩니다. 알파 채널이 0 또는 1 0% (이 완전히 투명 하 고 보이지 않는) 또는 100% (완전히 불투명 하 고 모든 계층 뒤에 컨트롤을 완전히 차단)에서 다릅니다.

**ColorFade** 함수는 색의 밝거나 짙은 버전을 반환합니다. 페이드의 양은 (색을 검정으로 완전히 어둡게)는-1에서 1 (색을 흰색으로 완전히 밝게 하)를 0 (색에 영향을 주지) 다릅니다.

## <a name="alpha-channel"></a>알파 채널

캔버스 앱에서 다른 앞에 컨트롤을 계층화 하 고 뒤에 있는 모든 컨트롤에 컨트롤의 투명도 지정할 수 있습니다. 결과적으로, 계층을 통해 색 결합 됩니다. 예를 들어이 다이어그램의 50% 알파 설정이 포함 된 세 가지 기본 색을 혼합 하는 방법을 보여 줍니다.

> [!div class="mx-imgBorder"]
> ![50%의 알파 설정이 포함 된 세 가지 기본 색](media/function-colors/alpha-primary.png)

알파 채널을 지 원하는 파일 형식에서 이미지도 혼합할 수 있습니다. 예를 들어,.jpeg 파일을 혼합할 수 없습니다. 하지만.png 파일을 혼합할 수 있습니다. 다음 그림은 동일한 빨간색이 빨간색으로 표시 하지만 이전 예제에서 녹색 및 파랑 색 다음과 같이 표시 됩니다 (원) 하는 대신 오류 표시 선이 50% 알파 채널을 사용 하 여.png 파일

> [!div class="mx-imgBorder"]
> ![파랑 및 녹색 원 앞에 50%의 알파 설정이 포함 된 빨간색 오류 표시선](media/function-colors/alpha-image.png)

지정 하는 경우는 **색** 열거형 값 이거나 빌드를 **ColorValue** 색 이름 또는 6 자리 16 진수 값을 사용 하 여 수식, 알파 설정을 100%가 완전히 불투명 합니다.

## <a name="syntax"></a>구문

**Color**.*ColorName*

- *ColorName* - 필수 항목입니다.  CSS(Cascading Style Sheet) 색 이름입니다. 이 항목의 끝에 열거 값의 목록이 표시 됩니다.

**ColorValue**( *CSSColor* )

- *CSSColor* - 필수 항목입니다.  CSS(Cascading Style Sheet) 색 정의입니다. 와 같은 두 이름을 지정할 수 있습니다 **OliveDrab**, 또는 16 진수 값을 같은 **#6b8e23** 또는 **#7fffd420**합니다. 16 진수 값 중 하나 # 형식일 수 있습니다*rrggbb* 또는 #*rrggbbaa*합니다.

**RGBA**( *Red*, *Green*, *Blue*, *Alpha* )

- *Red*, *Green*, *Blue* - 필수 항목입니다.  색 구성 요소 값 범위는 0 (채도 없음)에서 255 (전체 채도).
...
- *FadeAmount* - 필수 항목입니다.  -1에서 1 사이의 숫자입니다. -1에는 색을 검정으로 완전히 어둡게 하 고 0은 색에 영향을 주지 색을 흰색으로 완전히 밝게 하는 1입니다. 백분율-100에서 이용할 수 있습니다 100% %.
...
| **Color.Linen** |**ColorValue( "#faf0e6"&nbsp;)**<br>**ColorValue ("리넨"&nbsp;)** |**RGBA(&nbsp;250,&nbsp;240,&nbsp;230,&nbsp;1&nbsp;)** |![linen](./media/function-colors/color-linen.png) |
...
| **Color.Olive** |**ColorValue( "#808000"&nbsp;)**<br>**ColorValue ("올리브색"&nbsp;)** |**RGBA(&nbsp;128,&nbsp;128,&nbsp;0,&nbsp;1&nbsp;)** |![olive](./media/function-colors/color-olive.png) |
...
| **Color.SeaGreen** |**ColorValue( "#2e8b57"&nbsp;)**<br>**ColorValue ("해 록"&nbsp;)** |**RGBA(&nbsp;46,&nbsp;139,&nbsp;87,&nbsp;1&nbsp;)** |![seagreen](./media/function-colors/color-seagreen.png) |
...
| **Color.Transparent** |**ColorValue( "#00000000"&nbsp;)**<br>**ColorValue( "Transparent"&nbsp;)** |**RGBA(&nbsp;0,&nbsp;0,&nbsp;0,&nbsp;0&nbsp;)** |![투명 한](./media/function-colors/color-transparent.png) |
