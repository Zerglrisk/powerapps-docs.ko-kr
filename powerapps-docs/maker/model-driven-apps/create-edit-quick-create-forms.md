...
## <a name="allow-quick-create-property-form-behavior-for-activities"></a>활동에 대한 빨리 만들기 속성 양식 동작
...
다음 표에서는 통합 인터페이스 클라이언트에서 **빨리 만들기 허용** 속성이 *활성화*된 경우 기본적으로 표시되는 양식을 나타냅니다.
 
|양식이 액세스되는 위치  |표시되는 양식  |
|---------|---------|
|특정 활동 관련 표  | 빨리 만들기      |
|특정 활동 하위 표   |  빨리 만들기     |
|활동(activitypointer) 표     | 빨리 만들기     |
|활동(activitypointer) 관련 표   | 빨리 만들기    |
|활동(activitypointer) 하위 표  | 빨리 만들기    |
|전역 명령 모음 + 단추<sup>1</sup>    | 빨리 만들기    |
|타임라인 담벼락   | 빨리 만들기    |
|활동(activitypointer) 표   | 기본   |
|특정 활동 표    | 기본   |

<sup>1</sup>활동은 **빨리 만들기 허용** 속성이 활성화되면 전역 **만들기** 또는 **+ 새로 만들기** 단추에 나타납니다. 이 경우 빨리 만들기 양식이 존재하거나 기본 양식이 없는 경우 사용됩니다. **빨리 만들기 허용**이 비활성화된 경우 엔터티에 대한 항목이 표시되지 않습니다.

### <a name="classic-web-client-form-display-behavior"></a>클래식 웹 클라이언트 양식 표시 동작

다음 표에서는 클래식 웹 클라이언트에서 **빨리 만들기 허용** 속성이 *활성화*된 경우 기본적으로 표시되는 양식을 나타냅니다.
...
|특정 활동 하위 표   |  빨리 만들기     |
|활동(activitypointer) 표     | 기본     |
|활동(activitypointer) 관련 표   | 기본    |
|활동(activitypointer) 하위 표  | 기본    |
|전역 명령 모음 + 단추    | 기본    |
|특정 활동 표   | 기본    |

 #### <a name="classic-web-client-social-pane-behavior"></a>클래식 웹 클라이언트 소셜 창 동작
 
소셜 창은 **빨리 만들기 허용** 속성을 사용하지 않고 여기에 표시된 대로 서로 다른 활동 엔터티에 대해 다른 양식을 사용하기 때문에 특별한 경우입니다.
...
### <a name="solution-import-allow-quick-create-value-behavior"></a>솔루션 가져오기 빨리 만들기 허용 값 동작

솔루션에서 **빨리 만들기 허용** 속성의 값에 관계 없이 버전 8.2에서 솔루션을 가져올 때 작업, 전화 통화, 전자 메일 및 약속 엔터티가 기본 양식 표시 값으로 다시 설정되고 기본 양식이 표시됩니다. 이 경우 가져오기 후 해당 활동 엔터티에 대해 **빨리 만들기 허용** 옵션을 다시 *활성화됨*으로 다시 설정해야 합니다.
 
**빨리 만들기 허용**이 활성화된 엔터티에 버전 9.0 솔루션에서 사용자 지정이 있는 경우 가져오기 후 값이 변경되지 않습니다.  그러나 작업, 전화 통화, 전자 메일 및 약속 엔터티에 대해 **빨리 만들기 허용** 옵션을 *비활성화*로 설정한 경우 값을 활성화로 덮어쓰게 됩니다. 이 경우 가져오기 후 해당 활동 엔터티에 대해 **빨리 만들기 허용** 옵션을 다시 비활성화됨으로 다시 설정해야 합니다. 
