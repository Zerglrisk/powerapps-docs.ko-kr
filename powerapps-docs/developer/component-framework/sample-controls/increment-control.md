---
title: " 증가 구성 요소 | Microsoft Docs"
description: 증분 구성 요소 구현
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340187"
---
# <a name="implementing-increment-component"></a>증분 구성 요소 구현

이 샘플 구성 요소는 PowerApps 구성 요소 프레임 워크 및 오류 처리를 사용 하 여 데이터를 바인딩하는 방법을 보여 줍니다. 이 구성 요소는 런타임에 `Increment` 단추를 사용 하 여 입력란으로 렌더링 합니다. 텍스트 상자에 현재 값이 표시 되 고 `Increment` 단추를 클릭할 수 있습니다. 단추를 클릭할 때마다 입력란 내의 값이 1 씩 증가 합니다. 증가값은 원하는 수 만큼 변경할 수 있습니다.

이 구성 요소를 구현 하려면 먼저 [매니페스트](../manifest-schema-reference/manifest.md) 파일을 정의 하 고 TypeScript에서 사용자 지정 논리를 구현 해야 합니다.

> [!div class="mx-imgBorder"]
> ![증가 구성 요소](../media/increment-control.png "증가 구성 요소")
...
단추를 클릭 하면 입력란의 값이 1 씩 증가 합니다. 업데이트 된 값은 `notifyOutputChanged` 메서드를 통해 PowerApps 구성 요소 프레임 워크로 전달 됩니다.

> [!NOTE]
> 폼의 필드에 구성 요소를 구성할 때 증가값을 변경할 수 있습니다.

텍스트 상자에서 값을 편집 하 고, 유효한 정수인 경우 값을 PowerApps 구성 요소 프레임 워크로 업데이트 합니다. @No__t_0 단추를 계속 클릭 하 여 업데이트할 수 있습니다. 잘못 된 정수인 경우 오류 메시지가 팝업 됩니다.
