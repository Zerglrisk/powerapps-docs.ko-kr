...
- 관련성 검색은 단일 목록에서 관련성을 기준으로 정렬 된 여러 엔터티에 대해 빠르고 포괄적인 결과를 제공 합니다. Common Data Service 외부의 전용 검색 서비스 ([!INCLUDE[pn_Windows_Azure](../includes/pn-windows-azure.md)]으로 구동)를 사용 하 여 검색 성능을 향상 시킵니다. 
...
- **와일드 카드**: 예를 들어 * ski 또는 * ski @ no__t-0입니다. 
...
- 외부 인덱싱 및 @no__t 0 검색 기술을 사용 하 여 성능을 향상 시킵니다.  
...
- 노트의 텍스트, 전자 메일 첨부 파일 또는 약속을 포함 하 여 Common Data Service에 저장 된 문서에서 텍스트에 대 한 검색 결과를 찾을 수 있습니다. 검색에 지원 되는 파일 형식은 다음과 같습니다. PDF, Microsoft Office 문서, HTML, XML, ZIP, EML, 일반 텍스트 및 JSON입니다.  
...
- 옵션 집합 및 조회를 검색할 수도 있습니다. 예를 들어 이름에 **Pharmaceuticals** 이 있는 소매점 계정을 찾으려고 한다고 가정해 보겠습니다. **제약 Retail**을 검색할 때 검색 가능한 옵션 집합인 산업 필드와 일치 하는 항목이 있기 때문에 결과를 찾을 수 있습니다.  
 ...
 관련성 검색은 엔터티의 모든 필드에 있는 검색 용어의 단어와 일치 하는 항목을 찾았지만 전체 텍스트 검색을 사용 하도록 설정 된 경우에도 빠른 찾기 @ no__t-0에서는 검색 용어의 모든 단어를 한 필드에서 찾아야 합니다.  
...
2. 왼쪽에서 드롭다운 메뉴를 선택 하 여 **관련성 검색** 또는 **항목별 검색**사이를 전환 합니다.
   ...
   > 관련성 및 범주화 된 검색(media/switch-search.png "간") ![관련성 및 범주화 된 검색 전환 간 전환] 
...
### <a name="record-type-facets"></a>레코드 유형 패싯  
 특정 엔터티로 검색 결과의 범위를 좁히려면 **레코드 유형** 섹션 아래에서 엔터티를 선택 합니다.  
  ...
  > [!div class="mx-imgBorder"]
  > 검색 결과의 범위를 좁히는 데 사용 되는 검색 결과(media/relevance-search-record-type-facet.png "레코드 유형 패싯") ![의 범위를 제한 하는 레코드 유형 패싯]  
  
 특정 레코드 종류를 필터링 할 때 검색 결과에서 선택한 레코드와 관련 된 활동과 메모를 포함할 수 있습니다. 이렇게 하려면 **관련 메모 & 작업** 확인란을 선택 합니다. 활동 및 메모는 최상위 결과에 표시 됩니다.  
  
 
  > [!div class="mx-imgBorder"]
  > 검색 ![결과에서 레코드 유형과 관련 된 메모 및 활동 포함](media/relevance-search-record-type-facet-related-notes-activities.png "검색 결과의 레코드 유형과 관련 된 메모 및 활동을 포함") 합니다.  
 ...
 **개인 옵션 설정** 대화 상자에서 시스템 관리자나 고객이 사용 하도록 설정한 다른 패싯을 선택할 수도 있습니다. 사용자 설정이 기본 설정을 재정의 합니다. [!INCLUDE[proc_more_information](../includes/proc-more-information.md)] [검색에 대 한 패싯 및 필터 구성](#BKMK_ConfigureFacets)  
  
### <a name="text-based-facets"></a>텍스트 기반 패싯  
 모든 조회, 옵션 집합 및 레코드 형식은 텍스트 기반 패싯입니다. 예를 들어 텍스트 기반 패싯 소유자는 필드 값 목록과 해당 하는 개수로 구성 됩니다.  
 
  > [!div class="mx-imgBorder"]
  > 관련성 검색에서 ![텍스트 기반 패싯 관련 검색](media/relevance-search-text-based-facets.png "텍스트 기반 패싯")  
...
### <a name="date-and-time-facets"></a>날짜 및 시간 패싯  
 다른 패싯 처럼 날짜 및 시간 패싯을 사용 하 여 특정 시간에 대 한 검색 결과를 필터링 하 고 볼 수 있습니다. 값 범위를 선택 하려면 슬라이더를 끌거나 세로 열 중 하나를 선택 합니다.  
  ...
  > [!div class="mx-imgBorder"]
  > 관련성(media/relevance-search-date-time-facets.png "검색의 관련성 검색 날짜 및 시간 패싯에") ![대 한 날짜 및 시간 패싯]  
