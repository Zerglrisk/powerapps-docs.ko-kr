...
*공백*은 "값 없음" 또는 "알 수 없는 값"에 대한 자리 표시자입니다.  예를 들어 사용자가 선택을 하지 않은 경우 **[콤보 상자](../controls/control-combo-box.md)** 컨트롤의 **Selected** 속성은 *공백* 입니다. 많은 데이터 원본에서 NULL 값을 저장 하 고 반환할 수 있으며,이 값은 Power Apps에서 *비어*있는 것으로 표시 됩니다.

Power Apps의 모든 속성 또는 계산 된 값은 *비워 둘*수 있습니다.  예를 들어 부울 값은 일반적으로 **true** 또는 **false** 값 중 하나입니다.  그러나 이러한 두 가지 외에도 상태를 알 수 없음을 나타내는 *비워* 둘 수 있습니다.  이는 Microsoft Excel과 유사 합니다. 여기서 워크시트 셀은 내용 없이 공백으로 시작 되지만 **TRUE** 또는 **FALSE** 값을 포함할 수 있습니다. 언제 든 지 셀의 내용을 다시 지워 *빈* 상태로 되돌릴 수 있습니다.
...
**Isblank** 함수는 *빈* 값 또는 빈 문자열을 테스트 합니다.  값이 없는 경우 일부 데이터 소스 및 컨트롤에서 빈 문자열을 사용 하므로 응용 프로그램을 쉽게 만들 수 있도록 테스트에 빈 문자열이 포함 되어 있습니다.  *빈* 값을 특별히 테스트 하려면 **isblank**대신 `if( Value = Blank(), ...`를 사용 합니다.
...
`Coalesce( value1, value2 )`는 `If( Not IsBlank( value1 ), value1, Not IsBlank( value2 ), value2 )`에 더 간결 하 고 **value1** 및 **value2** 를 두 번 계산 하지 않아도 됩니다.  [ **If** 함수](function-if.md) 는 여기에 있는 경우와 같이 "else" 수식이 없으면 *blank* 를 반환 합니다.
...
| 수식 | 설명 | 결과 |
| --- | --- | --- |
| **병합 (&nbsp;Blank (),&nbsp;1&nbsp;)** |**Blank** 함수의 반환 값을 테스트하며, 항상 *공백* 값을 반환합니다. 첫 번째 인수는 *비어* *있으므로 비어 있지 않은 값과* 비어 있지 않은 문자열을 찾을 때까지 다음 인수를 사용 하 여 계산이 계속 됩니다. |**1** |
