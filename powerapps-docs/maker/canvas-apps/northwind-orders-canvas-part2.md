...
1. **트리 뷰** 창에서 **Screen1** 를 선택 하 여 주문 갤러리에 컨트롤을 실수로 추가 하지 않도록 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 트리 뷰 창에서 Screen1를 선택 합니다. @ no__t-1
...
1. 레이블 텍스트를 두 번 클릭 한 다음 **Northwind Orders**를 입력 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 제목 표시줄 @ no__t-1의 텍스트를 변경 합니다.
...
1. 수식 입력줄에서 양식의 **DataSource** 속성을이 값으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 편집 양식 컨트롤의 DataSource 속성을 설정 합니다. @ no__t-1
...
1. 오른쪽 가장자리 근처의 **속성** 탭에서 **필드 편집** 을 선택 하 여 **필드** 창을 엽니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 필드 @ no__t-1을 엽니다.

1. **필드** 창에서 **필드 추가**를 선택 하 고 **Customer** 및 **Employee** 필드의 확인란을 선택 합니다.

    > [!div class="mx-imgBorder"]
    > @no__t 사용자 및 직원 필드를 편집 양식 컨트롤 @ no__t-1에 추가 합니다.
...
1. 이러한 필드가 나타날 때까지 아래로 스크롤한 다음 해당 확인란을 선택 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t-편집 양식 컨트롤에 5 개 이상의 필드를 추가 @ no__t-1
...
1. **필드** 창 맨 아래에서 **추가**를 선택 하 고 **필드** 창을 닫습니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 편집 양식 컨트롤은 7 개의 필드 @ no__t-1을 표시 합니다.
...
1. 폼을 선택한 상태로 오른쪽 가장자리 근처의 **속성** 탭에서 폼의 열 수를 3에서 12로 변경 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t-편집 양식 컨트롤의 열 수 변경 @ no__t-1
...
1. 다른 컨트롤과 마찬가지로 핸들을 끌어 필드를 이동 하 고 크기를 조정 하 여 각 행에 지정 된 순서로 이러한 데이터 카드를 포함 하도록 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 필드 이동 및 크기 조정 @ no__t-1

    양식에서 필드를 정렬 하는 방법에 대 한 자세한 내용: [Canvas 앱에 대 한 데이터 양식 레이아웃을 이해](working-with-form-layout.md)합니다.
...
1. Shift 키를 누른 채 **수주일 데이터 카드의 시간** , 분 및 콜론 구분 기호 컨트롤을 선택 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t-주문 날짜 카드 @ no__t-1에서 시간 컨트롤을 선택 합니다.
...
1. 컨트롤의 **Visible** 속성을 **false**로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 표시 속성을 false로 설정 합니다. ](media/northwind-orders-canvas-part2/form-10.png)

1. [**날짜 선택**](controls/control-date-picker.md) 컨트롤의 크기를 조정 하 여 전체 날짜를 표시 합니다.

    > [!div class="mx-imgBorder"]
    > @no__t-날짜 선택 컨트롤 @ no__t-1의 크기를 조정 합니다.
...
1. **트리 뷰** 창의 **유료 날짜** 데이터 카드에서 시간 컨트롤을 선택 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t-유료 날짜 카드 @ no__t-1의 시간 제어 선택

1. 선택한 컨트롤의 **Visible** 속성을 **false**로 설정 합니다.

    > [!div class="mx-imgBorder"]
    > ![ 표시 속성을 false로 설정 합니다. ](media/northwind-orders-canvas-part2/form-13.png)
...
1. 요약 양식의 **Item** 속성을 다음 식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @ no__t-1 양식의 Item 속성을 @no__t 설정 합니다.

    양식은 앱 사용자가 목록에서 선택 하는 순서에 대 한 요약을 표시 합니다.

    > [!div class="mx-imgBorder"]
    > @no__t 목록에서 순서를 선택 하 여 @ no__t-1 형식의 개요를 표시 합니다.
...
1. 양식을 선택 하 고, 오른쪽 가장자리 근처의 **속성** 탭에서 **필드 편집** 을 선택한 다음, **주문 번호** 필드를 선택 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 주문 번호 필드 @ no__t-1을 선택 합니다.

1. **컨트롤 형식** 목록을 엽니다.

    > [!div class="mx-imgBorder"]
    > @no__t * * 컨트롤 형식 * * list @ no__t-1을 엽니다.

1. 텍스트 데이터 **보기** 카드를 선택 합니다.

    > [!div class="mx-imgBorder"]
    > ![ * * 텍스트 보기 * * 데이터 카드 @ no__t-1을 선택 합니다.
...
1. **삽입** 탭에서 **미디어** > **이미지**를 선택 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > 이미지 컨트롤이 있는 ![Employee 데이터 카드-1

1. 이미지 크기를 조정 하 고 콤보 상자의 오른쪽으로 이동 합니다.

    > [!div class="mx-imgBorder"]
    > ![ 이미지 컨트롤 이동 및 크기 조정 @ no__t-1
...
1. Alt 키를 누른 채 콤보 상자에서 다른 직원을 선택 하 여 그림이 변경 되는지 확인 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 직원의 그림 @ no__t-1을 표시할 직원을 선택 합니다.

## <a name="add-a-save-icon"></a>저장 아이콘 추가

1. **트리 뷰** 창에서 **Screen1**를 선택 하 고 **삽입** > **아이콘** > **검사**를 선택 합니다.

    > [!div class="mx-imgBorder"]
    > ![ 삽입 확인 표시 아이콘 @ no__t-1
...
1. **홈** 탭에서 아이콘의 **Color** 속성을 흰색으로 변경 하 고, 아이콘의 크기를 조정 하 고, 제목 표시줄의 오른쪽 가장자리 근처로 이동 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 저장 아이콘의 색, 크기 및 위치를 구성 합니다. @ no__t-1
...
1. **트리 뷰** 창에서 양식의 이름이 **Form1**인지 확인 하 고 아이콘의 **onselect** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 저장 아이콘의 OnSelect 속성 @ no__t-1을 설정 합니다.
...
1. 아이콘의 **DisplayMode** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 아이콘의 DisplayMode 속성 @ no__t-1 설정
...
1. 아이콘의 **DisabledColor** 속성을 다음 값으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 아이콘의 DisabledColor 속성 @ no__t-1 설정
...
1. 취소 아이콘의 **Onselect** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 취소 아이콘의 OnSelect 속성 @ no__t-1을 설정 합니다.
...
1. 취소 아이콘의 **DisplayMode** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 취소 아이콘의 DisplayMode 속성 @ no__t-1 설정
...
1. 추가 아이콘의 **Onselect** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 추가 아이콘의 OnSelect 속성 @ no__t-1을 설정 합니다.
...
1. 추가 아이콘의 **DisplayMode** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 추가 아이콘의 DisplayMode 속성 @ no__t-1 설정
...
1. 추가 아이콘의 **DisabledColor** 속성을이 값으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 추가 아이콘의 DisabledColor 속성 @ no__t-1 설정
...
1. 휴지통 아이콘의 **Onselect** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 휴지통 아이콘의 OnSelect 속성 @ no__t-1을 설정 합니다.
...
1. 휴지통 아이콘의 **DisplayMode** 속성을 다음 수식으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 휴지통 아이콘의 DisplayMode 속성 @ no__t-1을 설정 합니다.
...
1. 휴지통 아이콘의 **DisabledColor** 속성을이 값으로 설정 합니다.
    ...
    > [!div class="mx-imgBorder"]
    > ![ 휴지통 아이콘의 DisabledColor 속성 @ no__t-1을 설정 합니다.
...
- **Orders** 엔터티의 데이터를 표시 하는 폼입니다. **Form1. DataSource =** `Orders`
- 폼과 주문 갤러리 간 연결: **Form1. Item =** `Gallery1.Selected`
...
- 순서를 삭제 하는 아이콘: `Remove( Orders, Gallery1.Selected )`
